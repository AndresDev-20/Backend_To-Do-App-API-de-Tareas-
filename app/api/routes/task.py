...
async def add_task(data: task_schema.TaskSchema, db: Session = Depends(get_db)):
    return task_crud.create_task(db=db, data=data)


@router.put("/update/{task_id}")
async def task_update(task_id: int, data: task_schema.TaskSchema, db: Session = Depends(get_db)):
    db_task = task_crud.get_task_id(db, task_id=task_id)
    if db_task is None:
        raise HTTPException(status_code=404, detail="Tarea no encontrada")
